...
class GephiTopo (object):
    ...
    def send_full(self, client):
        ...
        for s in self.switches:
            if s in core.SRPConfig.core_list:
                out.append(an(s, kind='Core'))
            elif s in core.SRPConfig.tor_list:
                out.append(an(s,kind='Tor'))
        for e in self.links:
    ...
    def _handle_ForwardingFunction_HostEvent(self, event):
        h = event.entry["ipaddr"]
        s = event.entry["dpid"]
        ...
        if event.leave:
            ...
            if h not in self.hosts:
                ...
                if s in self.switches:
                    ...
                    log.debug("Missing switch")

    def _handle_core_GoingUpEvent(self,event):
        log.debug("Up...")
